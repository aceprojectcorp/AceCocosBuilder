...
//jambam 추가
// 메모리 관리를 편리하게 하기 위해 vector 변수 추가
#define CCB_MEMBERVARIABLEASSIGNER_GLUE_VECTOR(TARGET, MEMBERVARIABLENAME, MEMBERVARIABLETYPE, MEMBERVARIABLE, VECTOR) \
    if (pTarget == TARGET && 0 == strcmp(pMemberVariableName, (MEMBERVARIABLENAME))) { \
        MEMBERVARIABLETYPE pOldVar = MEMBERVARIABLE; \
        MEMBERVARIABLE = dynamic_cast<MEMBERVARIABLETYPE>(pNode); \
        CC_ASSERT(MEMBERVARIABLE); \
        if (pOldVar != MEMBERVARIABLE) { \
            CC_SAFE_RELEASE(pOldVar); \
            MEMBERVARIABLE->retain(); \
            VECTOR.push_back(MEMBERVARIABLE); \
        } \
        return true; \
    }


#define CCB_MEMBERVECTORASSIGNER_GLUE_VECTOR(TARGET, MEMBERVARIABLENAME, MEMBERVARIABLETYPE, MEMBERVECTOR, VECTOR) \
    if (pTarget == TARGET && 0 == strcmp(pMemberVariableName, (MEMBERVARIABLENAME))) { \
        MEMBERVARIABLETYPE pVar = dynamic_cast<MEMBERVARIABLETYPE>(pNode); \
        CC_ASSERT(pVar); \
        pVar->retain(); \
        MEMBERVECTOR.push_back(pVar); \
