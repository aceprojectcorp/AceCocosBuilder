...
, m_language(LANGUAGE_DEFAULT)
{
    ...
    this->setLanguage(pCCBReader->getLanguage());
...
bool CCBReader::init()
{
    ...
    //CCLog("CCBReader::init() %d", pActionManager->retainCount());
...
void CCBReader::setAnimationManager(CCBAnimationManager *pAnimationManager)
{
    ...
    //CCLog("CCBReader::init() %d", mActionManager->retainCount());
}
...
//CCNode** CCBReader::readNodeGraphFromFile(const char* pCCBFileName, CCObject* pOwner, int iArrayCnt)
//{
//    return this->readNodeGraphFromFile(pCCBFileName, pOwner, CCDirector::sharedDirector()->getWinSize(), iArrayCnt);
//}
...
CCNode* CCBReader::readNodeGraphFromFile(const char *pCCBFileName, CCObject *pOwner, const CCSize &parentSize)
{
    ...
    std::string newPath;
    //newPath = writablePath + strPath;
    newPath = writablePath + strCCBFileName;
//    CCLog("readNodeGraphFromFile().newPath = %s", newPath.c_str());
    if(CCFileUtils::sharedFileUtils()->isFileExist(newPath))
    {
        strPath = newPath;
//        CCLog("CCBReader.readNodeGraphFromFile.다운로드 받은 파일 로딩 : %s", strPath.c_str());
    }
    else
    {
        //CCLog("CCBReader.readNodeGraphFromFile.패키징된 파일 로딩 : %s", strPath.c_str());
    }
...
//CCNode** CCBReader::readNodeGraphFromFile(const char *pCCBFileName, CCObject *pOwner, const CCSize &parentSize, int iArrayCnt)
//{
//    if (NULL == pCCBFileName || strlen(pCCBFileName) == 0)
//    {
//        return NULL;
...
//    
//    
//    CCNode **ret = new CCNode*[iArrayCnt];
//    for (int i = 0; i < iArrayCnt; ++i) {
//        CCLog("%d",i);
//        ret[i] =  this->readNodeGraphFromData(data, pOwner, parentSize);
//    }
//
//    data->release();
//    
...
CCNode * CCBReader::readNodeGraph(CCNode * pParent) //jambam 원본
{
    /* Read class name. */
    ...
    //CCLog("memberVarAssignmentType = %d", memberVarAssignmentType);
    //---
    ...
    //---test
//    CCLog("node.contentSize = (%f, %f)", node->getContentSize().width, node->getContentSize().height);
    //---
    
    //haru - 하나의 ccb를 로딩할때 노드를 몇개나 생성하는지 체크하기 위함.
    this->mCCNodeLoaderLibrary->nodeCount++;
    
//    CCLog("클래스타입: %s", className.c_str());
    
    //haru - 만약 CCLabelTTF라면, 초기에 언어설정을 해주자.
    if (className.compare("CCLabelTTF") == 0)
    {
        CCLabelTTF *ttf = (CCLabelTTF*)node;
        ttf->setLanguage(m_language);
    }
    else if (className.compare("CCSprite") == 0)
    {
        CCSprite *sprite = (CCSprite*)node;
        sprite->setLanguage(m_language);
    }
    ...
    // Read properties
    ccNodeLoader->parseProperties(node, pParent, this); //jambam 원본
    ...
    if (isCCBFileNode)
    {
        ...
        //embeddedNode->setVisible(true); //jambam 원본
        embeddedNode->setVisible(ccbFileNode->isVisible()); //jambam 수정(툴에서 낱개ccb객체의 visible을 끌수도 있으니깐 수정한다.)
        //embeddedNode->ignoreAnchorPointForPosition(ccbFileNode->isIgnoreAnchorPointForPosition()); //jambam 요놈이 주석되어있으면 cocosBuilder와 다른 배치화면이 보여진다. 절대 주석하면안되. 왜 주석이 되어있었지? -_-;
        
        //haru - 위에서 parseProperties로 속성을 모두 읽은 상태이기 때문에, embeddedNode에 다시 속성을 강제로 셋팅 해야한다!!
        embeddedNode->enableChildrenOpacity(ccbFileNode->isEnableChildrenOpacity());
        
        //---test
//        CCLog("CCBReader.readNodeGraph.ccbFileNode.pos = (%f, %f)", ccbFileNode->getPosition().x, ccbFileNode->getPosition().y);
        //---
    ...
    //---jambam 추가
    //bool isCCScale9SpriteNode = (NULL == dynamic_cast<CCSprite*>(node)) ? false : true;
    //---
...
//                        CCLog("~~~~~9");
    ...
    //---jambam 추가
    bool isLayerNode = (NULL == dynamic_cast<CCLayer*>(node)) ? false : true;
    CCLayer* layerNode = (CCLayer*)node;
    if(isLayerNode)
    {
        layerNode = (CCLayer*)node;
        int aaa;
        aaa = 0;
//        CCLog("layNode 의 포인터 주소 = %p", layerNode);
//        bool val = layerNode->isIgnoreAnchorPointForPosition();
//        if(val) CCLog("val = true");
//        else CCLog("val = false");
    }
    //---
...
//    CCLog("numChildren = %d", numChildren);
    for(int i = 0; i < numChildren; i++) {
        CCNode * child = this->readNodeGraph(node); //jambam 원본
    ...
    //haru - childrenOpacity가 있을 때, 초기셋팅
    if (node->isEnableChildrenOpacity())
    {
        node->setOpacityChildren(node->getChildrenOpacity());
    }
...
CCBKeyframe* CCBReader::readKeyframe(int type)
{
    ...
    else if (type == kCCBPropTypeSpriteFrame)
    {
        ...
        //---test
//        CCLog("spriteSheet = %s", spriteSheet.c_str());
//        CCLog("spriteSheet.length() = %lu", spriteSheet.length());
//        CCLog("spriteFile = %s", spriteFile.c_str());
//        CCLog("spriteFile.length() = %lu", spriteFile.length());
        //---
...
bool CCBReader::readSoundKeyframesForSeq(CCBSequence* seq) {
    ...
    for(int i = 0; i < numKeyframes; ++i) {
        ...
        //---jambam
        //파일이 없는경우 파일 경로를 다운로드 받은 파일 경로로 셋팅해준다.
        if(access(soundFile.c_str(), F_OK) != 0)
        {
            std::string tmpPath = CCFileUtils::sharedFileUtils()->getWritablePath();
            
            soundFile = tmpPath + soundFile;
        }
        //---
