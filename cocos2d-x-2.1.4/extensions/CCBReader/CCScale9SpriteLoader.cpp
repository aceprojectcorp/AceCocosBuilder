...
#define PROPERTY_ANTIALIASING "antialiasing"
...
//    CCLog("[0] plist : CCScale9SpriteLoader::onHandlePropTypeSpriteFrame()");
...
//    CCLog("[0] plist : CCScale9SpriteLoader::onHandlePropTypeFloat()");
...
// [sj] anti-aliasing 설정 추가
void CCScale9SpriteLoader::onHandlePropTypeCheck(CCNode * pNode, CCNode * pParent, const char* pPropertyName, bool pCheck, CCBReader * pCCBReader) {
    if(strcmp(pPropertyName, PROPERTY_ANTIALIASING) == 0) {
        ((CCScale9Sprite*)pNode)->setAntialiasing(pCheck);
    } else {
