...
void CCSpriteLoader::onHandlePropTypeCheck(CCNode * pNode, CCNode * pParent, const char* pPropertyName, bool pCheck, CCBReader * pCCBReader) {
    if(strcmp(pPropertyName, PROPERTY_ANTIALIASING) == 0) {
        ((CCSprite *)pNode)->setAntialiasing(pCheck);
    } else {
