...
//haru - 파티클 시작/정지 추가
void CCParticleSystemQuadLoader::onHandlePropTypeCheck(CCNode * pNode, CCNode * pParent, const char* pPropertyName, bool pCheck, CCBReader * pCCBReader) {
    if(strcmp(pPropertyName, PROPERTY_ACTIVE) == 0) {
        ((CCParticleSystemQuad *)pNode)->setActive(pCheck);
    } else {
